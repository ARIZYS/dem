...
LOGO_PNG      = os.path.join(RESOURCES_DIR, "icon.png")
...
# Цвета в соответствии с руководством по стилю
COLOR_WHITE        = "#FFFFFF"   # Основной фон
COLOR_SECOND       = "#00FFFF"   # Дополнительный фон
COLOR_ACCENT       = "#0000FF"   # Акцентирование внимания (целевое действие)
COLOR_DISCOUNT_ROW = "#008080"   # Скидка > 15%
...
class LoginDialog(QDialog):
    def __init__(self):
        ...
        # Установка фона и стиля для всего диалога
        ...
        # Акцентирующий цвет для целевого действия
...
class ProductFormDialog(QDialog):
    def __init__(self, product_id=None):
        ...
        self.photo_label.setStyleSheet("border:1px solid #546F94; background-color: white;")
        ...
        # Акцентирующий цвет для целевого действия (сохранение)
...
class OrdersDialog(QDialog):
    def __init__(self, user_data: UserInfo, parent=None):
        ...
        title.setStyleSheet("font-size:24px; font-weight:700; font-family: Calibri; color: black;")
        top.addWidget(title)
        top.addStretch()
        ...
        # Акцентирующий цвет для целевого действия (добавление)
        ...
        for i, x in enumerate(rows):
            ...
            for col in range(6):
                if self.table.item(i, col):
                    self.table.item(i, col).setForeground(QColor("black"))
...
class ProductsWindow(QMainWindow):
    def __init__(self, user_data: UserInfo):
        ...
        # Установка шрифта Calibri для главного окна
        font = QFont("Calibri", 10)
        self.setFont(font)
        ...
        header = QGridLayout()
        logo = QLabel()
        # Установка логотипа на главной форме (без искажения)
        if os.path.exists(ICON_PNG):
            pix = QPixmap(ICON_PNG)
            # Не искажаем: сохраняем пропорции, не меняем изображение и цвет
            logo.setPixmap(pix.scaled(120, 70, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
        title = QLabel("ООО «Мебель» — Список товаров")
        title.setStyleSheet("font-size:28px; font-weight:700; font-family: Calibri; color: black;")
        self.role_label = QLabel(f"Роль: {self._role_caption(user_data.role_name)}")
        self.role_label.setStyleSheet("color: black;")
        ...
        self.btn_orders.setStyleSheet(f"""
            background-color: {COLOR_SECOND}; 
            color: black; 
            font-family: Calibri; 
            font-size: 10pt;
        ...
        label_search.setStyleSheet("color: black;")
        fl.addWidget(label_search)
        self.search_edit = QLineEdit()
        self.search_edit.setStyleSheet("color: black; background-color: white;")
        fl.addWidget(self.search_edit)
        label_range = QLabel("Диапазон скидки:")
        label_range.setStyleSheet("color: black;")
        fl.addWidget(label_range)
        self.discount_range_combo = QComboBox()
        self.discount_range_combo.addItems(DISCOUNT_RANGES)
        self.discount_range_combo.setStyleSheet("color: black; background-color: white;")
        fl.addWidget(self.discount_range_combo)
        label_sort = QLabel("Сортировка:")
        label_sort.setStyleSheet("color: black;")
        fl.addWidget(label_sort)
        ...
        self.sort_combo.setStyleSheet("color: black; background-color: white;")
        ...
        # Акцентирующий цвет для целевого действия (добавление товара)
        self.btn_add.setStyleSheet(f"""
            background-color: {COLOR_ACCENT}; 
            color: black; 
            font-family: Calibri; 
        ...
        # Установка основного фона (белый) и черного текста для всего окна
        self.setStyleSheet(f"""
            QMainWindow {{ background-color: {COLOR_WHITE}; }}
            QWidget {{ background-color: {COLOR_WHITE}; }}
            QLabel {{ color: black; }}
            QLineEdit {{ color: black; background-color: white; }}
            QComboBox {{ color: black; background-color: white; }}
            QComboBox QAbstractItemView {{
    ...
    def apply_filters(self):
        ...
        if self.is_manager_or_admin():
            ...
            if discount_range == "0–10.99%":
                ...
                data = [x for x in data if 11 <= Decimal(str(x.get("discount_percent") or 0)) < 15]
            elif discount_range == "15% и более":
                data = [x for x in data if Decimal(str(x.get("discount_percent") or 0)) >= 15]
    ...
    def fill_table(self, rows):
        ...
        for i, row in enumerate(rows):
            ...
            for c, v in enumerate(values, start=1):
                ...
                item.setForeground(QColor("black"))  # Всегда черный текст
            ...
            if discount > 0:
                ...
                price_item.setForeground(QColor("black"))  # Черный текст для зачеркнутой цены

            # Применение цветов фона строк в соответствии с правилами
            ...
            elif discount_val > DISCOUNT_THRESHOLD:
                # Скидка больше 15% - фон #008080
...
def main():
    ...
    app.setStyle("Fusion")
    # Установка шрифта Calibri для всего приложения
    app.setFont(QFont("Calibri", 11))
    
    # Установка иконки приложения, если возможно
    if os.path.exists(ICON_ICO):
